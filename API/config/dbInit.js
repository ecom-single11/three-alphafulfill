...
const tables = [
    ...
    {
        name: 'products_sizeGuideImage_col',
        sql: `
            ALTER TABLE \`products\`
            ADD COLUMN IF NOT EXISTS \`sizeGuideImage\` VARCHAR(500) DEFAULT NULL
            AFTER \`galleryMedia\`;
        `
    },

    // PRODUCTS: ensure all required columns exist (catch-all for older DBs)
    {
        name: 'products_all_cols_v2',
        sql: `
            ALTER TABLE \`products\`
            ADD COLUMN IF NOT EXISTS \`shortDescription\`    VARCHAR(500) DEFAULT NULL,
            ADD COLUMN IF NOT EXISTS \`resultTag\`           VARCHAR(100) DEFAULT NULL,
            ADD COLUMN IF NOT EXISTS \`badge\`               VARCHAR(100) DEFAULT NULL,
            ADD COLUMN IF NOT EXISTS \`benefits\`            JSON         DEFAULT NULL,
            ADD COLUMN IF NOT EXISTS \`whyLoveIt\`           JSON         DEFAULT NULL,
            ADD COLUMN IF NOT EXISTS \`keyIngredients\`      JSON         DEFAULT NULL,
            ADD COLUMN IF NOT EXISTS \`howToUse\`            LONGTEXT     DEFAULT NULL,
            ADD COLUMN IF NOT EXISTS \`additionalDetails\`   LONGTEXT     DEFAULT NULL,
            ADD COLUMN IF NOT EXISTS \`consumerCareDetails\` LONGTEXT     DEFAULT NULL,
            ADD COLUMN IF NOT EXISTS \`faqs\`                JSON         DEFAULT NULL,
            ADD COLUMN IF NOT EXISTS \`promoImage\`          VARCHAR(500) DEFAULT NULL,
            ADD COLUMN IF NOT EXISTS \`promoReel\`           VARCHAR(500) DEFAULT NULL,
            ADD COLUMN IF NOT EXISTS \`galleryMedia\`        JSON         DEFAULT NULL,
            ADD COLUMN IF NOT EXISTS \`sizeGuideImage\`      VARCHAR(500) DEFAULT NULL,
            ADD COLUMN IF NOT EXISTS \`isFeatured\`          TINYINT(1)   NOT NULL DEFAULT 0,
            ADD COLUMN IF NOT EXISTS \`sortOrder\`           INT(11)      NOT NULL DEFAULT 0;
        `
    },
